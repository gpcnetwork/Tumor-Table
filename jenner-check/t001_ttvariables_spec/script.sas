/* Source: scr/TUMOR TABLE QC SCRIPT.sas (GPC Tumor-Table), ttvariables specification.
   The NAACCR tumor-table variable spec, verbatim from the repo, then summarised.
   obs=max is set so the full reference spec loads (this is fixed lookup data,
   not the bulk input that obs=100 is meant to cap). */
options obs=max;
...
title1 'PCORnet Tumor Table v1.2 specification: variable count by flag and timing';
proc freq data = ttvariables;
	tables flag * timing / nopercent norow nocol;
run;

title1 'Priority variables in the tumor table specification';
proc print data = ttvariables noobs;
	where flag = 'PRIORITY';
	var variable flag timing;
run;
